...
- Auf zwei intakten Dächern ohne Gorilla stehen **eine Fahne und eine Windhose**, um 10 % verkleinert; mindestens eine davon befindet sich zwischen beiden Gorillas. **Eine weitere Fahne und eine weitere Windhose** stehen am Strand. Alle vier reagieren auf denselben tatsächlichen Wind; bei Windstille hängen sie herunter.
...
- Das Replay nutzt denselben WebGL-Kontext. Ohne Replay sind es drei Zeichenaufrufe je Bild (Stadt, Stadtleben, Spieleffekte); währenddessen kommen zwei für die Nahaufnahme hinzu. Das dekorative Stadtleben wird in der Nahaufnahme ausgelassen. Ein zusätzlicher temporärer Stadtpuffer wird beim Schließen wieder freigegeben.
...
- **Linksziehen** mit Maus oder Ziehen mit Finger dreht die Kamera, das Mausrad zoomt. **Rechtsziehen** oder **Shift + Linksziehen** verschiebt den Blickpunkt über die Stadt; **I / J / K / L** bewegen ihn bei fokussiertem Spielfeld vorwärts, links, rückwärts und rechts. Die Verschiebung bleibt pro Spieler und nach Neuladen gespeichert. **Übersicht** setzt die Kamera zurück, **Am Wurf** zeigt die Stadt aus Richtung des aktiven Gorillas. Die Schaltflächen **+ / −** zoomen auch auf Touchscreens. Bei fokussiertem Spielfeld funktionieren **A / D**, **+ / −** und **Pos1**.
...
18 Fahrzeuge (Autos, Taxis und Busse) folgen festen Fahrspuren, zwei Boote fahren vor der Küste, fünf Menschen spazieren auf der Promenade und ein Zeppelin zieht hinter der Skyline vorbei. Strandliegen, Sonnenschirme und Badegäste ergänzen den Strand. Jede neue Runde erhält eine zufällige Uhrzeit zwischen Sonnenaufgang und Sonnenuntergang (05:30–19:00 in Fünf-Minuten-Schritten). Manuell ist der gesamte 24-Stunden-Tag von 00:00 bis 23:55 wählbar; Nachtzeiten werden nicht automatisch ausgelost. Nachts liegt die Sonne unter dem Horizont, das Sonnenlicht entfällt und kühles Umgebungslicht hält die Stadt mit ihren eingeschalteten Leuchten erkennbar. Im Spielfeld steht die Uhrzeit über den Kameratasten. Der dortige Schieberegler ändert die Beleuchtung live, auch mit den Pfeiltasten sowie Pos1/Ende. Sonnenstand, Lichtfarbe, Himmel, Wasserreflex und Fensterbelegung passen zusammen; die sichtbare Sonne und ihr Trefferbereich verwenden dieselbe Position. Die Uhrzeit verwendet einen eigenen Zufallsaufruf unabhängig vom Spielzufall. Der Spielstand enthält die gewählte Uhrzeit; beim Neuladen der Spielseite bleibt sie unverändert erhalten, auch nach manueller Nachtwahl.

Noch stärkeres indirektes Umgebungslicht und ein geringerer Anteil direkten Sonnenlichts halten auch die abgewandten Fassaden erkennbar, ohne den Sonnenstand oder die Gebäudeschatten zu verlieren. Bei Sonnenauf- und -untergang schalten Straßen- und Promenadenlaternen, Hauseingangsleuchten, Scheinwerfer und Rücklichter der Fahrzeuge sowie Positionslichter der Boote ein. Kleine, weich auslaufende Lichtflächen beleuchten den Boden unter Laternen, an Eingängen und vor Fahrzeugen. Tagsüber bleiben diese Flächen unsichtbar. Die Lichtquellen nutzen die vorhandenen Stadt- und Fahrzeugpuffer; es gibt keine zusätzlichen Zeichenaufrufe oder Schattenkarten je Lampe. Die Lichtflächen sind eine günstige Darstellung, keine vollständige Simulation lokaler Lichtausbreitung.
...
Ab 22 Uhr gehen nach und nach mehr Wohnungslichter aus; zwischen 2 und 4 Uhr bleibt nur etwa ein Fünftel der abendlichen Fensterbeleuchtung an. Zum Morgen steigt die Belegung wieder. Straßenlaternen, Fahrzeuglichter und sanft wechselnde Neon-Schilder bleiben eingeschaltet. Größere Lichtflächen, zusätzliche Hauseingangsleuchten und das spielerfarbene Licht der Gorillas auf nahen Flächen machen die Nacht besser lesbar. Explosionen besitzen bei Tag und Nacht einen hellen Kern, einen expandierenden Feuerring, leuchtende Funken und einen lokal begrenzten, rasch abklingenden Lichtschein auf benachbarten Flächen. Der zusätzliche Blitz entfällt bei reduzierter Bewegung; Replay und Hauptansicht verwenden jeweils ihr eigenes Explosionsalter. Alle Effekte bleiben in den vorhandenen Zeichenaufrufen.
...
Außerhalb der Stadt setzen bewaldete Hügel, Weiden und kleine Höfe die Landschaft fort; alles bleibt statische Geometrie.

Die Dekoration beeinflusst weder Kollisionen noch Spielzufall, Punkte oder gespeicherte Matches. Es gibt keine Verkehrssimulation: Fahrzeuge folgen getrennten Rundkursen beziehungsweise entfernten Ein- und Ausfahrten. Pause und reduzierte Bewegung frieren das Stadtleben ohne Nachholsprung ein. Gebäudezerstörung erzeugt keine neuen Akteure. Die Routen berücksichtigen auch die kleineren Stadtgrundrisse älterer 24- und 48-Block-Spielstände.

Die 26 Akteure teilen sich einen wiederverwendeten GPU-Puffer mit 3.424 Dreiecken und genau einem zusätzlichen Zeichenaufruf. Modelle werden einmal erzeugt; pro Bild werden nur ihre Positionen und Normalen in einem vorhandenen Array aktualisiert. Im Stillstand entfallen auch diese Uploads. Es gibt keine neuen Bibliotheken, externen Grafikdateien oder Partikelsysteme.

`tests/ambient3d.html` bietet Nahansichten von Promenade, Skyline und Umland, Pause, reduzierte Bewegung, Rundenwechsel, Gebäudeschäden, einen Zeitsprung für Fensterwechsel und einen Vergleich mit ausgeschaltetem Stadtleben. Es zeigt CPU-Median, 95. Perzentil, Bildrate, Dreieckszahl, Puffer-Uploads, Tageszeit und Schattenaktualisierungen. Lokal im Codex-Browser wurden bei 60 FPS etwa 0,1 ms zusätzliche CPU-Zeit im Median und 0,2 ms im 95. Perzentil gemessen; dies ist keine Leistungsgarantie für andere Geräte.
