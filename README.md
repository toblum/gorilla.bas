...
Sieben Fahrzeuge (Autos, Taxis und Busse) folgen festen Fahrspuren, zwei Boote fahren vor der Küste, vier Menschen spazieren auf der Promenade und ein Zeppelin zieht hinter der Skyline vorbei. Strandliegen, Sonnenschirme und Badegäste ergänzen den Strand. Kühle Umgebungsbeleuchtung, warmes Sonnenlicht und einzelne selbstleuchtende Fenster geben den Fassaden mehr Tiefe.
...
Die 14 Akteure teilen sich einen wiederverwendeten GPU-Puffer mit 1.252 Dreiecken und genau einem zusätzlichen Zeichenaufruf. Modelle werden einmal erzeugt; pro Bild werden nur ihre Positionen und Normalen in einem vorhandenen Array aktualisiert. Im Stillstand entfallen auch diese Uploads. Es gibt keine neuen Bibliotheken, Texturen, Echtzeitschatten oder Partikelsysteme.

`tests/ambient3d.html` bietet Nahansichten von Promenade und Skyline, Pause, reduzierte Bewegung, Rundenwechsel, Gebäudeschäden und einen Vergleich mit ausgeschaltetem Stadtleben. Es zeigt CPU-Median, 95. Perzentil, Bildrate, Dreieckszahl und Puffer-Uploads. Lokal im Codex-Browser wurden bei 60 FPS etwa 0,1 ms zusätzliche CPU-Zeit im Median und 0,2 ms im 95. Perzentil gemessen; dies ist keine Leistungsgarantie für andere Geräte.

### Prüfung der 3D-Erweiterung

`npm test` umfasst 88 Tests. Die zusätzlichen 3D-Tests prüfen unter anderem 100 Städte, räumliche Flugbahnen, beidseitige Treffer, Tiefenversatz, Wind in zwei Achsen, Kollisionen bei 20/60/144 FPS, durchfliegbare Schäden, Sonnenladung, Selbsttreffer, Matchende, Session-Wiederherstellung, gedrehte Trefferflächen, den äußeren Blockring und die Windanzeigerplatzierung, gespeicherte Kameraansichten, weiche Zugwechsel und unverfälschte Replay-Flugbahnen.
